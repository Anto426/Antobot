...
class log {
    ...
    emojiDelete(emoji, tag) {
        let embedmsg = new logembed(channel.guild, channel);
        embedmsg.init().then(() => {
            this.sendlog(embedmsg.emojiDelete(emoji), channel.guild, tag).catch(() => { });
        }).catch(() => { this.console.log("Errore nell'inizializzare l'embed", "red") });
    }
    ...
    guildMemberUpdate(member, changedprop, tag) {
        let embedmsg = new logembed(channel.guild, channel);
        embedmsg.init().then(() => {
            this.sendlog(embedmsg.guildMemberUpdate(member, changedprop), channel.guild, tag).catch(() => { });
        }).catch(() => { this.console.log("Errore nell'inizializzare l'embed", "red") });
    }
    ...
    inviteCreate(invite, tag) {
        let embedmsg = new logembed(channel.guild, channel);
        embedmsg.init().then(() => {
            this.sendlog(embedmsg.inviteCreate(invite), channel.guild, tag).catch(() => { });
        }).catch(() => { this.console.log("Errore nell'inizializzare l'embed", "red") });
    }
