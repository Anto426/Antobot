...
class SynchronizationManager {
  ...
  async synchronizeAll() {
    ...
    try {
      ...
      for (const [index, guild] of guilds.entries()) {
        const guildLogPrefix = `[Sync ${index + 1}/${guilds.length}][${guild.name}]`;
        try {
          await this.synchronizeGuild(guild, guildLogPrefix);
        } catch (guildError) {
          BotConsole.error(`${guildLogPrefix} Errore non gestito durante la sincronizzazione.`, guildError);
        }
      }
    } catch (error) {
      ...
      BotConsole.success(`SINCRONIZZAZIONE GLOBALE TERMINATA IN ${(endTime - startTime) / 1000}s.`);
    }
  }
  ...
  async synchronizeGuild(guildOrId, logPrefix = "") {
    ...
    BotConsole.success(`${guildLogPrefix} Sincronizzazione completa terminata.`);
  }
  ...
  async pruneObsoleteGuilds() {
    ...
    for (const dbGuild of dbGuilds) {
      if (!allClientGuildIds.has(dbGuild.ID)) {
        BotConsole.warning(`[Prune] Gilda "${dbGuild.NOME}" (${dbGuild.ID}) non più accessibile. Rimozione dati...`);
  ...
  async #synchronizeRolesForGuild(guild, logPrefix) {
    ...
    const clientRoleIds = new Set(guild.roles.cache.filter(r => r.id !== guild.id).map(r => r.id));
    ...
    for (const role of guild.roles.cache.values()) {
      ...
      await SqlManager.synchronizeRole({
        id: role.id, name: role.name, color: role.color, guildId: guild.id,
      });
    }
  ...
  async #synchronizeMembersForGuild(guild, logPrefix) {
    ...
    for (const member of fetchedMembers.values()) {
      ...
      const memberLogName = member.user.globalName || member.user.displayName || member.user.username;
      await SqlManager.synchronizeGlobalMember({ id: member.id, globalName: memberLogName });
      await SqlManager.ensureGuildMemberAssociation(guild.id, member.id);
      
      const roleIdsSet = new Set(member.roles.cache.filter(r => r.id !== guild.id).map(r => r.id));
      await SqlManager.synchronizeMemberRolesForGuild(member.id, guild.id, roleIdsSet);
    }
    BotConsole.info(`${logPrefix} Sincronizzazione Membri (${fetchedMembers.size}) COMPLETATA.`);
  }
