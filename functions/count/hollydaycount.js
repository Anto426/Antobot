...
function nexhollyday() {
    ...
    try {
        ...
        do {
            if (new Date().getTime(optionsdate) < new Date(new Date().getFullYear(), chollyday.holidays[cont].date.mouth, chollyday.holidays[cont].date.day).getTime(optionsdate)) {
...
async function updatecount(festa, channelcount) {
    ...
    try {
        setInterval(async () => {
            let timereminig = new Date(new Date().getFullYear(), festa.mouth, festa.day).getTime(optionsdate) - new Date().getTime(optionsdate).getTime(optionsdate)
...
async function sendcongratulations(festa) {
    setInterval(() => {
        ...
        let timereminig = new Date(new Date().getFullYear(), festa.date.mouth, festa.date.day).getTime(optionsdate) - new Date().getTime(optionsdate)
        if (timereminig <= 0) {
