...
export default {
  ...
  async execute(guild) {
    try {
      ...
      BotConsole.error(`Errore durante l'evento guildCreate per "${guild.name}" (${guild.id})`, error);
    }
  },
  ...
  async sendWelcomeMessage(guild) {
    ...
    if (guild.systemChannel && guild.systemChannel.permissionsFor(me).has(PermissionsBitField.Flags.SendMessages)) {
      channelToSend = guild.systemChannel;
    } else {
      const textChannels = guild.channels.cache.filter(c => 
        c.type === 0 && c.permissionsFor(me).has(PermissionsBitField.Flags.SendMessages)
      );
    ...
    if (channelToSend) {
      try {
        ...
        try {
          ...
          BotConsole.error(`Fallito invio messaggio di benvenuto in "${guild.name}" (canale e DM).`, dmError);
        }
      }
    } else {
      BotConsole.warning(`Nessun canale adatto trovato in "${guild.name}" per inviare il messaggio di benvenuto.`);
    }
  }
};